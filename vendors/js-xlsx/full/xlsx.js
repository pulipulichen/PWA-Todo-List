...
async function parse_zip(file, options) {
  var blob = file;
	prep_blob(blob, 0);

  //console.log('parse_zip', 1)
...
var utf8readAsync = async function utf8reada(orig) {
  var JSXLSXsleep = function  (ms) {
    if (typeof(ms) !== 'number') {
      ms = 1
    }
  ...
  //var waitInterval = 100
    ...
    if (i % waitInterval === 0) {
      ...
      //}
      //else {
        //await JSXLSXsleep(10)
      //}
    }
    
    if (i % (waitInterval * 100) === 0) {
      console.log('push cache', i)
      ...
      //outArray.push(String.fromCharCode(f))
      continue; 
    }
		e = orig.charCodeAt(i++);
		if (c < 240) { 
      out += String.fromCharCode(((c & 15) << 12) | ((d & 63) << 6) | (e & 63)); 
      //outArray.push(String.fromCharCode(((c & 15) << 12) | ((d & 63) << 6) | (e & 63)))
      continue; 
    }
		f = orig.charCodeAt(i++);
		w = (((c & 7) << 18) | ((d & 63) << 12) | ((e & 63) << 6) | (f & 63))-65536;
		out += String.fromCharCode(0xD800 + ((w>>>10)&1023));
		out += String.fromCharCode(0xDC00 + (w&1023));
    //outArray.push(String.fromCharCode(0xD800 + ((w>>>10)&1023)))
    //outArray.push(String.fromCharCode(0xDC00 + (w&1023)))
...
var JSXLSXsleep = function  (ms) {
  ...
  return new Promise((resolve) => {
	...
	return async function pcx(d, _opts, isAsync) {
		var opts = _opts || {};
		if(DENSE != null && opts.dense == null) opts.dense = DENSE;
		var str = xlml_normalize(d);
		var state = [], tmp;
...
async function parse_odsAsync(zip, opts) {
	opts = opts || ({});
	var ods = !!safegetzipfile(zip, 'objectdata');
...
async function parse_zipAsync(zip, opts) {
	make_ssf(SSF);
	opts = opts || {};
	fix_read_opts(opts);
...
async function read_zipAsync(data, opts) {
  var zip, d = data;
	var o = opts||{};
	if(!o.type) o.type = (has_buf && Buffer.isBuffer(data)) ? "buffer" : "base64";
  //console.log('zip_read', 1)
	zip = zip_read(d, o);
  //console.log('zip_read', 2)
	return await parse_zipAsync(zip, o);
  //return parse_zip(zip, o);
}
...
async function readAsync(data, opts) {
	reset_cp();
	if(typeof ArrayBuffer !== 'undefined' && data instanceof ArrayBuffer) return readSync(new Uint8Array(data), opts);
	var d = data, n = [0,0,0,0], str = false;
	var o = opts||{};
...
//console.log(parse_zip)
XLSX.parse_zipAsync = parse_zipAsync;
XLSX.read = readSync; //xlsread
XLSX.readAsync = readAsync; //xlsread
